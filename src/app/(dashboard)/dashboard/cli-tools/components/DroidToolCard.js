...
export default function DroidToolCard({
  ...
  const getConfigStatus = () => {
    ...
    const currentConfig = droidStatus.settings?.customModels?.find(m => m.id === "custom:Api2K-0");
  ...
  useEffect(() => {
    if (droidStatus?.installed && !hasInitializedModel.current) {
      ...
      const customModel = droidStatus.settings?.customModels?.find(m => m.id === "custom:Api2K-0");
      if (customModel) {
          ...
          {!checkingDroid && droidStatus?.installed && (
            <>
              <div className="flex flex-col gap-2">
                {/* Current Base URL */}
                {droidStatus?.settings?.customModels?.find(m => m.id === "custom:Api2K-0")?.baseUrl && (
                  <div className="flex items-center gap-2">
