...
export default function MediaProviderDetailPage() {
  ...
  const matchesKind = kind === "audio"
    ? (kinds.includes("tts") || kinds.includes("stt") || kinds.includes("music"))
    : kinds.includes(kind);
  if (!matchesKind) return notFound();
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      {/* Models - only for non-audio kinds */}
      {kind !== "audio" && <ModelsCard providerId={id} kindFilter={kind} />}

      {/* Example — per kind */}
      {kind === "embedding" && <EmbeddingExampleCard providerId={id} />}
      {kind === "audio" && <TtsExampleCard providerId={id} />}
