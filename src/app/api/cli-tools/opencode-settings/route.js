...
export async function PATCH(request) {
  try {
    ...
    if (clearActiveModel === true) {
      ...
      if (config.model?.startsWith("api2k/")) {
        config.model = "";
      }
    }
...
// DELETE - Remove Api2K provider or specific models from config
...
// If specific model provided, remove just that model
    if (modelToRemove && config.provider?.["api2k"]?.models) {
      ...
      } else if (config.model === `api2k/${modelToRemove}`) {
        // If removed model was active, switch to first remaining model
        const remainingModels = Object.keys(config.provider["api2k"].models);
        config.model = `api2k/${remainingModels[0]}`;
      }
    } else {
      // No specific model - remove entire api2k provider
      if (config.provider) delete config.provider["api2k"];
      if (config.model?.startsWith("api2k/")) delete config.model;
    }

    // Remove subagent configuration
    if (config.agent?.explorer?.model?.startsWith("api2k/")) {
