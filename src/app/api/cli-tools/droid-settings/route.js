...
const hasApi2KConfig = (settings) => {
  ...
  return settings.customModels.some(m => m.id === "custom:Api2K-0");
};
...
// POST - Update Api2K customModels (merge with existing settings)
export async function POST(request) {
  try {
    ...
    // Remove existing Api2K config if any
    settings.customModels = settings.customModels.filter(m => m.id !== "custom:Api2K-0");

    // Normalize baseUrl to ensure /v1 suffix
    const normalizedBaseUrl = baseUrl.endsWith("/v1") ? baseUrl : `${baseUrl}/v1`;

    // Add new Api2K config
    const customModel = {
      model: model,
      id: "custom:Api2K-0",
      index: 0,
